...
public class SignUpActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ImageView googleButton = findViewById(R.id.google_icon);
        ...
        ivShowSignUpPassword.setOnClickListener(v -> {
            ...
            Log.d(TAG, "SignUp password visibility toggled: " + isPasswordVisible);
        });
        ...
        ivShowConfirmPassword.setOnClickListener(v -> {
            ...
            Log.d(TAG, "Confirm password visibility toggled: " + isConfirmPasswordVisible);
        });

        // Create Account Button Click
        btnCreateAccount.setOnClickListener(v -> {
            String email = etSignUpUsername.getText().toString().trim().toLowerCase();
            ...
            if (email.isEmpty() || name.isEmpty() || password.isEmpty() || confirmPassword.isEmpty()) {
                ...
                Log.w(TAG, "Sign up attempt with empty fields");
                return;
            }
            ...
            if (!email.endsWith(".vn")) {
                Toast.makeText(SignUpActivity.this, "Please enter an email ending with .vn", Toast.LENGTH_SHORT).show();
                Log.w(TAG, "Email does not end with .vn: " + email);
                return;
            }
            if (password.length() < 6) {
                Toast.makeText(SignUpActivity.this, "Password must be at least 6 characters", Toast.LENGTH_SHORT).show();
                Log.w(TAG, "Password too short: length=" + password.length());
                return;
            }
            ...
            apiManager.signUp(request, new ApiResponseListener<SignUpResponse>() {
                ...
                public void onSuccess(SignUpResponse response) {
                    if (response != null && response.getMetaData() != null && response.getMetaData().getMetadata() != null
                        ...
                        Log.d(TAG, "Sign up successful: " + new Gson().toJson(response));
                        Toast.makeText(SignUpActivity.this, "Sign up successful", Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent(SignUpActivity.this, MainActivity.class);
                        intent.putExtra("user_avatar", avatar);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                        LocalBroadcastManager.getInstance(SignUpActivity.this).sendBroadcast(
    ...
    private void loginWithGoogle(String idToken) {
        Log.d(TAG, "Sending idToken to server for Google login");
